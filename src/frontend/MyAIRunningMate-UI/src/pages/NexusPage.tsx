...
import type { NexusFormRequest } from '../types/nexus/nexusFormRequest';
...
export const NexusPage = () => {
  const [formData, setFormData] = useState<NexusFormRequest>({
    primary_goal: '5k',
  ...
  const [plan, setPlan] = useState<TrainingPlanViewResponse | null>(null);
  ...
  const updateEventField = (
    ...
    field: 'description' | 'distance_metres',
    value: string | number
  ) => {
    setPlan((prev) => {
      if (!prev) return prev;
      const events = prev.events.map((event, index) => {
        ...
        if (field === 'distance_metres') {
          ...
          return { ...event, distance_metres: Number.isNaN(metres) ? 0 : Math.max(0, metres) };
        }
    ...
    setPlan((prev) => {
      ...
      const events = prev.events.map((event, index) => {
        ...
        return {
          ...
          distanceMetres: exerciseType === 'Rest' ? 0 : event.distance_metres,
  ...
  const updateExerciseSubtype = (originalIndex: number, exerciseSubtype: string) => {
    setPlan((prev) => {
      ...
      const events = prev.events.map((event, index) => {
        if (index !== originalIndex) return event;
        const exerciseType = normalizeExerciseType(event.exercise_type);
        return {
          ...event,
          exerciseType,
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (normalized.training_plan.start_date && normalized.events) {
        const startMs = new Date(normalized.training_plan.start_date).getTime();
        const totalDays = (formData.schedule_length_weeks + 1) * 7;
        
        const eventMap = new Map<string, TrainingPlanEventResponse>();
        normalized.events.forEach((ev) => {
          const dateKey = new Date(ev.event_date).toISOString().split('T')[0];
          eventMap.set(dateKey, ev);
        });

        const completeEvents: TrainingPlanEventResponse[] = [];
        ...
        normalized.events = completeEvents;
      }
  ...
  const organizedWeeks = useMemo(() => {
    ...
    for (let w = 0; w < totalPlanWeeks; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        const event = plan.events[originalIndex];
    ...
    <div className="max-w-7xl mx-auto space-y-8 px-4">
      ...
      <div className="flex justify-between items-end border-b border-slate-800 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-black tracking-tighter uppercase italic">Nexus AI Mate</h2>
            <p className="text-slate-400 font-medium">Configure your AI Training Plan.</p>
          </div>
        </div>
      </div>
      ...
      {plan && (
        <section className="p-6 rounded-2xl border border-slate-700 bg-slate-900/50 space-y-6">
          <div>
            <h3 className="text-2xl font-bold text-white">{plan.training_plan.title}</h3>
            <p className="text-slate-400 mt-1">
              Microcycles: {formData.schedule_length_weeks} Weeks Phase Training + 1 Week Autonomous Recovery
            </p>
            {plan.training_plan.description && (
              <p className="text-slate-300 mt-2 text-sm max-w-4xl leading-relaxed">{plan.training_plan.description}</p>
            )}
          </div>
                ...
                <div key={week.weekNumber} className="flex gap-4 items-stretch">
                  ...
                  <div className="grid grid-cols-7 flex-1 gap-2">
                    {week.days.map(({ date, event, originalIndex }) => {
                      ...
                      const exerciseSubtype = normalizeExerciseSubtype(exerciseType, event.exercise_subtype);
                        ...
                        >
                          <div className="flex justify-between items-start gap-1">
                            ...
                            {!isRest && (
                              <span className="text-[10px] bg-blue-500/10 px-1.5 py-0.5 rounded text-slate-400 font-mono">
                                {formatDistance(event.distance_metres)}
                              </span>
                            )}
                          </div>
                          ...
                          <div>
                            <input
                              ...
                              step={100}
                              disabled={isRest}
                              value={isRest ? '' : event.distance_metres}
                              onChange={(e) => updateEventField(originalIndex, 'distance_metres', e.target.value)}
